...
def generate_certificate_pdf(attempt):
    ...
    p.setFillColor(COLOR_DARK)
    p.drawCentredString(center_x, height - 205, "Recgetup Music Private Limited")
    ...
    p.setFont("Times-Roman", 11)
    p.setFillColor(COLOR_GRAY)
    p.drawCentredString(center_x, height - 365, "organized by Recgetup Music Private Limited.")
    ...
    p.drawCentredString(center_x - 170, sig_y - 12, "Recgetup Music Pvt. Ltd.")
...
class EmailThread(threading.Thread):
    def __init__(self, subject, message, recipient_list, from_email=None, html_message=None, fail_silently=False):
